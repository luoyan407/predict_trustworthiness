...
def anneal(global_step, warmup=100, anneal=5000):
    return torch.clip((torch.tensor(global_step).float() - warmup) / anneal, 0.0, 1.0)
    # return 1.0 * max(min((global_step - warmup) / anneal, 1.0), 0.0)